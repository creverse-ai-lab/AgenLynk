...
- live `pushEvent`는 도착 순서를 유지한다. sequence로 재정렬하지 않는다.
- 같은 session의 같은 finite sequence는 중복으로 버리고 넣지 않는다.
- session이 live에서 빠지면 이벤트가 있을 때만 history로 아카이브하고, history는 `ts` 다음 `sequence`로 정렬한 뒤 최근 `maxEventsPerSession`개만 남긴다.
- live cap도 같은 한도로 가장 오래된 이벤트를 버린다.
- `subscription_replay_truncated`는 이벤트/notice로 올리지 않는다. `subscription_error`는 SSE `kind: "state"`로만 올라가고 `kind: "notice"`는 없다.
