...
## 8. Model control and concurrency hardening

### SC-21 Main-selected model

1. Main이 `session_open`에 provider와 `model`을 함께 전달한다.
2. session config를 지원하는 agent와 process model을 사용하는 agent를 각각 연다.
3. session config agent에는 다음 prompt에서 다른 `model`을 지정하고, process model session에는 다른 모델 변경을 요청한다.
4. session을 저장·복구한 뒤 현재 model을 확인한다.

기대 결과: session config agent에는 prompt 전 `session/set_config_option`이 호출되고, process model provider는 모델별 ACP process를 사용하며 기존 session의 중간 변경은 새 session을 요구하는 오류가 된다. 응답과 checkpoint에는 agent가 확정한 현재 model이 기록된다. 지원하지 않거나 잘못된 model은 조용히 fallback하지 않는다.

현재 상태: `자동화됨` — mock ACP config round-trip, Grok command model 인자와 실제 provider smoke를 검증한다.
