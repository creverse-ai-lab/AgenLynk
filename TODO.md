...
- [x] lifecycle, resource limit, agent update의 안전한 runtime config 17개를 동적으로 렌더링한다.
...
- [ ] 오래된 session/event가 retention 정책대로 정리되면서 active/pinned session은 보존되는 UI acceptance를 추가한다.
...
- [ ] `inspect`, `stage`, `validate`, `activate`, `rollback`, `prune` 계약을 가진 updater를 정의한다.
- [ ] 활성화 전 manifest, checksum, runtime smoke test와 Gateway API 호환성을 검증한다.
- [ ] previous known-good runtime을 기록하고 활성화 후 health mismatch 시 자동 롤백한다.
- [ ] session, Task, Inbox가 활성 상태면 updater가 activation/restart를 보류한다.
- [ ] CLI와 Swift UI가 동일 updater 결과 JSON을 사용한다.
- [ ] 개발자용 `git pull` update와 라이트 사용자용 runtime update를 명확히 분리한다.
...
- [ ] 선택 가능한 번들 sample renderer를 제공한다.
- [ ] DMG seed가 앱 번들에서 직접 실행되지 않는 packaging test를 자동 release gate에 추가한다.
- [ ] 메뉴바/popover, Dashboard, Pet이 동일 activity 상태를 표시하는 projection 테스트를 추가한다.
...
- [x] **G0 Core CI** — 2026-08-10 `b4016b1`에서 `npm test` 171개와 `npm run macos:test`를 로컬 재실행해 통과했다.
...
- [x] **G5 Config safety** — 안전 config 17개의 list/set/reset, env lock과 active-work restart blocker가 자동 테스트를 통과한다.
...
- [ ] **G8 Update/rollback** — bad stage는 current를 바꾸지 않고 post-activation health 실패는 previous로 복구한다.
