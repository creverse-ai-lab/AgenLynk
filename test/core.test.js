...
test("SessionStore spills a complete oversized result to a bounded artifact", async () => {
  const directory = await mkdtemp(join(tmpdir(), "acp-artifact-result-"));
  const artifactStore = new ArtifactStore({ root: directory, maxFileBytes: 1_000, maxTotalBytes: 2_000 });
  const store = new SessionStore({ maxTextBytes: 9, artifactStore });
  const session = store.create({ id: "artifact-result", ownerRootId: "main-a" });
  const full = "prefix-😀😀-tail";
  try {
    for (const part of ["prefix-", "\uD83D", "\uDE00", "😀-tail"]) store.appendResultText(session, part);
    const artifact = store.finalizeResult(session);
    assert.equal(await readFile(artifact.path, "utf8"), full);
    assert.equal(artifact.bytes, Buffer.byteLength(full));
    assert.equal(artifact.complete, true);
    assert.equal(artifact.truncated, false);
    assert.ok(Buffer.byteLength(session.resultText) <= 9);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});

test("ArtifactStore enforces file and directory byte caps", async () => {
  const directory = await mkdtemp(join(tmpdir(), "acp-artifact-cap-"));
  try {
    const artifacts = new ArtifactStore({ root: directory, maxFileBytes: 8, maxTotalBytes: 10 });
    const first = artifacts.create("session", "result");
    first.append("1234567890");
    first.finalize();
    assert.equal(first.metadata().bytes, 8);
    assert.equal(first.metadata().truncated, true);
    const second = artifacts.create("session", "terminal");
    second.append("abcd");
    second.finalize();
    assert.equal(second.metadata().bytes, 2);
    assert.equal(second.metadata().truncated, true);
    const exhausted = artifacts.create("session", "result");
    exhausted.append("x");
    exhausted.finalize();
    assert.equal(exhausted.metadata().path, null);
    assert.equal(exhausted.metadata().complete, true);
    assert.equal(exhausted.metadata().truncated, true);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});

test("bounded NDJSON reader accepts split lines and rejects oversized frames", async () => {
  ...
  assert.match(overflow.message, /exceeds 8 bytes/);
});
...
test("ACP client caps retained terminals per session", async () => {
  const client = new AcpClient(
    {
      provider: "mock",
      command: process.execPath,
      args: [capabilityAgent],
      permissionPolicy: "auto_approve"
    },
    { permissionPolicy: "auto_approve", maxTerminalsPerSession: 1 }
  );
  try {
    await client.start();
    const session = await client.sessionNew({ cwd: process.cwd(), permissionPolicy: "auto_approve" });
    assert.deepEqual(
      await client.sessionPrompt({ sessionId: session.sessionId, prompt: "terminal-limit" }),
      { stopReason: "limited" }
    );
  } finally {
    await client.stop();
...
test("ACP client caps concurrent unanswered permission requests", async () => {
  const client = new AcpClient(
    {
      provider: "mock",
      command: process.execPath,
    ...
    { permissionPolicy: "ask", maxPendingRequestsPerSession: 1 }
  );
  const requests = [];
  try {
    await client.start();
    ...
    client.onSessionUpdate(session.sessionId, (update) => {
      if (update.sessionUpdate === "permission_request") requests.push(update.requestId);
    });
    const turn = client.sessionPrompt({ sessionId: session.sessionId, prompt: "two-writes" });
    while (requests.length === 0) await new Promise((resolve) => setImmediate(resolve));
    await new Promise((resolve) => setTimeout(resolve, 20));
    client.respondPermission(requests[0], null, session.sessionId);
    assert.deepEqual(await turn, { stopReason: "rejected" });
